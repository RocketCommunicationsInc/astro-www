const onInput = (/** @type {string} */ value) => {
			...
			(/** @type {HTMLElement} */ element) => {
				...
				const glossaryItems = [
					...
					const categories = itemElement
						...
						.split(',')
						.map((category) => category.replace('_', ' ')) // replace underscore with space for search performance with human typing
					const tags = itemElement
						.querySelector('metadata.glossary-metadata')
						.textContent.split(', ')
						.map((tag) => tag.replace('_', ' ')) // replace underscore with space for search performance with human typing
