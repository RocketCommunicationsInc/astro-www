{
	...
	/** Search Form. */
	let formElement = /** @type {HTMLFormElement} */ (
		document.currentScript.previousSibling
	)

	/** Search Terms Control. */
	let formControl = /** @type {HTMLInputElement} */ (
		formElement.querySelector('.-control')
	)

	/** Search Terms Clear Button. */
	let formClear = /** @type {HTMLInputElement} */ (
		formElement.querySelector('.-clear')
	)
	...
	// this performs the search filter in the case where the user has pressed the back button so they can return to their searched state
				...
				(/** @type {HTMLElement} */ element) => {
					...
					const glossaryItems = [
						...
						const name = itemElement
							.querySelector('.glossary-item-name')
							.textContent.toLowerCase()
							.trim()
						const description = itemElement
							.querySelector('.glossary-item-description')
							.textContent.toLowerCase()
							.trim()
						...
						const tags = itemElement
							.querySelector('metadata.glossary-metadata')
							.textContent.split(', ')
							.map((tag) => tag.replaceAll('_', ' ')) // replace underscore with space for search performance with human typing
