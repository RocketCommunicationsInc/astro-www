...
const handleSlotHideClick = (target: HTMLButtonElement) => {
	...
	if (target.ariaSelected === 'false') {
		...
		// @ts-ignore
		const slotNode = globalThis.$actualSlots.find((item: { slot: string | null }) => item.slot === slotName)
...
function getEnabledSlots() {
	...
	// @ts-ignore
	globalThis.$actualSlots = actualSlots
	...
	if (globalThis.$slots.length > 0) {
		const ul = h('<ul>')
		let slotLi: HTMLElement
		globalThis.$slots.map(slot => {
...
window.addEventListener('load', () => {
	...
	// @ts-ignore
	globalThis.$slots = [ ...slots ] as any[]
	getEnabledSlots()
})

// change slot disabled/enabled when variant changes
