...
export default async function handler(request, context) {
  ...
  if (sessionCookie) {
    ...
    const gatedPath = `/gated${url.pathname}`;

    console.log(`[Content Gate] Rewriting to: ${gatedPath}`);

    // Use context.rewrite to internally serve the gated content
    // This serves the gated content without changing the URL
    return context.rewrite(gatedPath);
  } else {
    console.log('[Content Gate] User not authenticated - serving public content');
